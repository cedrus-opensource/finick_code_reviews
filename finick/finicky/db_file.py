...
def AssertType_DbTextFile( o ):
    ...
    if type(o) != type(rhs):
        raise FinickError("AssertType_DbTextFile failed. DbTextFile was required, but instead we got: " + incoming_type)
...
class DbTextFile(object):
    ...
    def _fail_setter(self, value):
        raise FinickError("The value you are trying to assign to in DbTextFile is read-only.")
    ...
    def _initialize_from_file( self, finick_config ):
        ...
        if not db_found:
            ...
            db_committed = finicky.gitting.git_repo_contains_committed_file( finick_config, os.path.abspath(expected_db) )
